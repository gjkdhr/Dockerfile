...
function config_passwd(){
	...
	mysql -uroot -predhat -e "GRANT ALL PRIVILEGES ON testdb.* TO 'testdb'@'%' IDENTIFIED BY 'testdbadmin' WITH GRANT OPTION; FLUSH PRIVILEGES;"
	mysql -uroot -predhat -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY 'dbadmin' WITH GRANT OPTION;FLUSH PRIVILEGES;"
